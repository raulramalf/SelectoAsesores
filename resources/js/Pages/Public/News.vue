...
const noticiasFiltradas = computed(() => props.noticias.filter(n => {
    const matchFiltro = filtroActivo.value === 'Todas' || n.fuente === filtroActivo.value
...
<template>
    <PublicLayout>
        ...
        <section class="news-grid-section" style="min-height:60vh;">
            ...
            <div v-if="noticiasFiltradas.length > 0" class="news-grid">
                <div v-for="noticia in noticiasFiltradas" :key="noticia.id"
                    ...
                    <div style="padding:18px 16px;">
                        <span style="font-size:9px;font-weight:500;letter-spacing:.16em;text-transform:uppercase;color:#E2CBAE;display:block;margin-bottom:7px;">{{ noticia.source }}</span>
                        <a :href="noticia.source_url" target="_blank" rel="noopener noreferrer" style="text-decoration:none;">
                            <h3 style="font-family:'Instrument Serif',serif;font-size:16px;font-weight:400;color:#fff;line-height:1.35;margin-bottom:8px;transition:color .2s;"
                                ...
                                @mouseleave="$event.target.style.color='#fff'">{{ noticia.title }}</h3>
                        </a>
