...
class AppointmentController extends Controller
{
    ...
    public function confirm($token) {
        $appointment = \App\Models\Appointment::where('token', $token)->firstOrFail();
        $appointment->update(['status' => 'confirmada', 'token' => null]);
    
        return "<h1>¡Cita confirmada! Te esperamos el día " . $appointment->date . "</h1>";
    }
}