        ...
        return Inertia::render('Admin/Dashboard', [
            ...
            'solicitudes' => Appointment::with('client')
                ...
                ->map(fn($c) => [
                    'id'          => $c->id,
                    'texto'       => $c->reason,
                    'nombre'      => $c->client?->name ?? $c->guest_name ?? 'Contacto Externo',
                    'es_invitado' => is_null($c->client_id),
                    'estado'      => $c->status ?? 'pendiente',
                    'fecha'       => $c->date ? Carbon::parse($c->date)->format('d/m/Y') : now()->format('d/m/Y'),
                ]),
        ...
        return Inertia::render('Admin/Citas', [
            'citas' => Appointment::with('client')
                ...
                ->map(fn($c) => [
                    ...
                    'nombre'      => $c->client?->name ?? $c->guest_name ?? $c->name ?? '—',
                    'email'       => $c->client?->email ?? $c->guest_email ?? $c->email ?? '—',
                    'phone'       => $c->client?->phone ?? $c->guest_phone ?? $c->phone ?? '—',
                    'es_invitado' => is_null($c->client_id),
                    'razon'       => $c->reason ?? $c->razon ?? 'Asesoría',
                    'fecha'       => $c->date ? Carbon::parse($c->date)->format('d/m/Y') : now()->format('d/m/Y'),
                    'date'        => $c->date,
                    'time'        => $c->time ?? '—',
